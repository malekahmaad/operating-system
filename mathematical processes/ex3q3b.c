...
void* newAdd(void* pols);
void* newSub(void* pols);
...
int* add(int *polinome1,int *polinome2,int size){
    pthread_t threadData[size];
    struct data x[size];
    int* result=(int*) malloc(sizeof (int)*size);
    for(int i=0;i<size;i++){
        x[i].pol1=polinome1[i];
        x[i].pol2=polinome2[i];
        pthread_create(&threadData[i],NULL,newAdd,(void*)&x[i]);
    }
    for(int i=0;i<size;i++){
        pthread_join(threadData[i],NULL);
    }
    for(int i=0;i<size;i++){
        result[i]=x[i].result;
    }
...
void* newAdd(void* pols){
    ...
    struct data* x=(struct data*)pols;
    x->result=x->pol1+x->pol2;
...
int* sub(int *polinome1,int *polinome2,int size){
    ...
    for(int i=0;i<size;i++){
        x[i].pol1=polinome1[i];
        x[i].pol2=polinome2[i];
        pthread_create(&threadData[i],NULL,newSub,(void*)&x[i]);
    }
    for(int i=0;i<size;i++){
        pthread_join(threadData[i],NULL);
    }
